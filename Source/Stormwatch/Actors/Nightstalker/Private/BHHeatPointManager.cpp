...
DEFINE_LOG_CATEGORY_CLASS(UBHHeatPointManager, LogHeatPointManager);

UBHHeatPointManager::UBHHeatPointManager()
{
}

void UBHHeatPointManager::Initialize(UBHNightstalkerDirector* Subsystem)
{
	if (!Subsystem) { return; }
...
void UBHHeatPointManager::Deinitialize()
{
	Deactivate();
	FlushHeatPoints();
}

void UBHHeatPointManager::Activate()
{
	if (const UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(HeatPointProcessorTimerHandle, this, &UBHHeatPointManager::UpdateHeatPointLifeTime, 1.0f, true);
		UE_LOG(LogHeatPointManager, Verbose, TEXT("Activated heat point manager."))
	}
}

void UBHHeatPointManager::Deactivate()
{
	if (const UWorld* World = GetWorld())
	{
...
void UBHHeatPointManager::RegisterHeatPoint(AHeatPoint* Instance)
{
	if (!Instance) { return; }
...
void UBHHeatPointManager::UnregisterHeatPoint(AHeatPoint* Instance)
{
	if (!Instance) { return; }
...
void UBHHeatPointManager::UpdateHeatPoints(TArray<FHeatPointOverlapData>& OverlapData)
{
...
void UBHHeatPointManager::SortHeatPointsByHeatValue()
{
...
void UBHHeatPointManager::FlushHeatPoints()
{
...
void UBHHeatPointManager::UpdateHeatPointLifeTime()
{
...
void UBHHeatPointManager::RemoveZeroHeatPoints()
{
	if (HeatPoints.IsEmpty()) { return; }
...
bool UBHHeatPointManager::IsActive() const
{
	if (const UWorld* World = GetWorld())
	{
		if (World->GetTimerManager().IsTimerActive(HeatPointProcessorTimerHandle))
...
AHeatPoint* UBHHeatPointManager::GetHottestHeatPoint()
{
