...
DEFINE_LOG_CATEGORY_CLASS(UBHPlayerFlashlightComponent, LogPlayerFlashlightComponent)

/** Sets default values for this component's properties. */
UBHPlayerFlashlightComponent::UBHPlayerFlashlightComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	PrimaryComponentTick.bStartWithTickEnabled = false;
}

void UBHPlayerFlashlightComponent::OnRegister()
{
	Super::OnRegister();
...
void UBHPlayerFlashlightComponent::BeginPlay()
{
	if (!Mesh || !Camera || !Movement) { return; }
	Super::BeginPlay();
}
...
/** Called every frame. */
void UBHPlayerFlashlightComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UBHPlayerFlashlightComponent::UpdateMovementAlpha(const float DeltaTime)
{
	const bool bIsMoving = Movement->Velocity.Length() > 1;
	if (MovementAlpha != static_cast<int8>(bIsMoving))
	{
			constexpr float InterpolationSpeed = 4;
			MovementAlpha = FMath::FInterpTo(MovementAlpha, bIsMoving, DeltaTime, InterpolationSpeed);
	}
}

FRotator UBHPlayerFlashlightComponent::GetFlashlightFocusRotation() const
{
...
FRotator UBHPlayerFlashlightComponent::GetFlashlightSwayRotation() const
{
	FRotator Rotation = FRotator();
	
	const EBHPlayerGroundMovementType MovementType = Movement->GetGroundMovementType();
...
FRotator UBHPlayerFlashlightComponent::GetSocketRotationWithOffset(const FName Socket, const EBHPlayerGroundMovementType MovementType) const
{
	...
	const bool bIsSprinting = MovementType == EBHPlayerGroundMovementType::Sprinting;

	/** Constants to tweak the flashlight's orientation when sprinting or walking. */
	const float PitchOffset = MovementType == EBHPlayerGroundMovementType::Sprinting ? 70.0f : 80.0f;
	const float PitchMultiplier = MovementType == EBHPlayerGroundMovementType::Sprinting ? 1.45f : 0.75f;
	const float YawMultiplier = MovementType == EBHPlayerGroundMovementType::Sprinting ? 0.175f : 0.075f;
		
	/** Select an 2-dimensional vector to compensate for socket rotation deviation between different animations. */
	FVector2D Offset;
	switch(MovementType)
	{
	case EBHPlayerGroundMovementType::Idle: Offset = Configuration->IdleOffset;
		break;
	case EBHPlayerGroundMovementType::Walking: Offset = Configuration->WalkingOffset;
		break;
	case EBHPlayerGroundMovementType::Sprinting: Offset = Configuration->SprintingOffset;
		break;
...
void UBHPlayerFlashlightComponent::SetFlashlightEnabled(bool Value)
{
...
bool UBHPlayerFlashlightComponent::IsFlashlightEnabled() const
{
	if (Flashlight)
	{
...
void UBHPlayerFlashlightComponent::CleanupComponent()
{
	if (Flashlight)
	{
		Flashlight->SetVisibility(false);
...
void UBHPlayerFlashlightComponent::OnUnregister()
{
	CleanupComponent();
	Super::OnUnregister();
}

void UBHPlayerFlashlightComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	CleanupComponent();
	Super::EndPlay(EndPlayReason);
}

void UPlayerFlashlightConfiguration::ApplyToFlashlightComponent(const UBHPlayerFlashlightComponent* Component)
{
